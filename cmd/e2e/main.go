...
func main() {
	...
	// Build + distribute payload from node 0
	ctx, cancel := contextWithTimeout(30 * time.Second)
	cid, payload := buildAndDistribute(ctx, sims[0], *payloadSize, *chunkSize, *fanout, *keepLocal)
	cancel()
	if (cid == block.CID{}) {
		log.Fatalf("failed to build payload")
	}

	// Give DHT/provider records a moment to settle
	time.Sleep(*settle)
